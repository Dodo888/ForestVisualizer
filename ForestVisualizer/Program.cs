...
namespace ForestVisualizer
{
    ...
    class FormForestVisualizer : Form, IForestVisualizer
    {
        ...
        private void DrawMap(Graphics graphics)
        {
            ...
            foreach (var citizen in players)
            {
                graphics.DrawImage(images["Citizen"+citizen.Id.ToString()],
                    ...
                    scaleH, scaleH);
                graphics.DrawImage(Visible(citizen.Target) ? images["Finish"] : images["FinishFog"],
                    citizen.Target.Y * scaleH, citizen.Target.X * scaleH,
                    scaleH, scaleH);
        ...
        {
            float statsWidth = Width - map.GetLength(0) * scaleH;
            graphics.DrawString("STATS:",
                new Font("Calibri", (int)(statsScale / 2), FontStyle.Bold), Brushes.White,
                new RectangleF(map.GetLength(0) * scaleH, 0,
                (int)(Width - map.GetLength(0) * scaleH), statsScale), StringFormat.GenericDefault);
            string[] statsHeader = new string[] { "Name", "HP", "DTF", "PTW" };
            string[][] playersStats = new string[2][];
            foreach (var citizen in players)
            {
                playersStats[citizen.Id - 1] = new string[] {citizen.Name, citizen.Hp.ToString(), 
            ...
            for (int i = 0; i < 3; i++)
                for (int j = 0; j < 4; j++)
                {
                    ...
                    if (j==0)
                        cell = new RectangleF(map.GetLength(0) * scaleH + j * statsWidth / 6, (i + 1) * statsScale,
                            2 * statsWidth / 6, statsScale);
                    else
                        cell = new RectangleF(map.GetLength(0) * scaleH + (j+1) * statsWidth / 6, (i + 1) * statsScale,
        ...
        private void DrawLeaderboard(Graphics graphics, float statsScale)
        {
            ...
            graphics.DrawString("LEADERBOARD:",
                new Font("Calibri", (int)(statsScale / 2), FontStyle.Bold), Brushes.White,
                new RectangleF(map.GetLength(0) * scaleH, 4 * statsScale,
                (int)(Width - map.GetLength(0) * scaleH), statsScale), StringFormat.GenericDefault);
            ...
            for (int i = 0; i < 9; i++)
                for (int j = 0; j < 7; j++)
                {
                    RectangleF cell = new RectangleF();
                    if (j == 0)
                        cell = new RectangleF(map.GetLength(0) * scaleH + j * statsWidth / 9, (i + 5) * statsScale,
                            statsWidth / 9, statsScale);
                    if (j == 1)
                        cell = new RectangleF(map.GetLength(0) * scaleH + j * statsWidth / 9, (i + 5) * statsScale,
                            2* statsWidth / 9, statsScale);
                    if (j > 1)
                        cell = new RectangleF(map.GetLength(0) * scaleH + (j + 1) * statsWidth / 9, (i + 5) * statsScale,
                            statsWidth / 9, statsScale);
            ...
            foreach (var position in leaderBoard.OrderByDescending(x => (x.Points)).ToArray())
            {
                ...
                for (int j = 0; j < 7; j++)
                {
                    ...
                    graphics.DrawString(pos[j],
                            new Font("Calibri", (int)(statsScale / 3), FontStyle.Bold), players.Select(x => x.Name).Contains(position.Name) ? Brushes.Red : Brushes.White,
                            new RectangleF(map.GetLength(0) * scaleH + j * statsWidth / 9, (count + 5) * statsScale,
                    ...
                    if (j>1)
                        graphics.DrawString(pos[j],
                            new Font("Calibri", (int)(statsScale / 3), FontStyle.Bold), players.Select(x => x.Name).Contains(position.Name) ? Brushes.Red : Brushes.White,
                            new RectangleF(map.GetLength(0) * scaleH + (j+1) * statsWidth / 9, (count + 5) * statsScale,
                            statsWidth / 9, statsScale), format);
                }
                count++;
            }
        ...
        private void RecieveInfo()
        {
            ...
            else
            {
                ...
                if (move.GameOver)
                {
                    ...
                    foreach (var player in players)
                        if (player.CurrentPosition == player.Target)
                        {
                            winner = player.Name;
                            hasWinner = true;
                        }
                    ...
                    else
                    {
                        foreach (var position in leaderBoard)
                        {
                            if (position.Name == players[0].Name || position.Name == players[1].Name)
                            {
                                position.Draws++;
                            }
                        }
